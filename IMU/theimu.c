...
void addr_set(uint8_t addr) {
 ...
 void imu_init() {
     ...
     SYSCTL_RCGCGPIO_R |= 0x2; // enable for A
     while (!(SYSCTL_PRGPIO_R) & 0x2); // port A

     GPIO_PORTB_DEN_R |= 0xC0; // change port
     GPIO_PORTB_AFSEL_R &= ~0xC0; // change port
     GPIO_PORTB_DIR_R |= 0xC0; // change port 

     // Set PB6 to HIGH 
     GPIO_PORTB_DATA_R |= 0x40; // change port 
